...
export default function ReliabilityAnalysis({ questions, answers }: ReliabilityAnalysisProps) {
    ...
    const analyzeReliability = () => {
        ...
        ncItems.forEach(q => {
            ...
            else if (q.content.includes("'그렇지 않다'에 응답")) target = 2; // "전혀 그렇지 않다" 보다 먼저 체크되지 않도록 주의 필요 없나? '전혀 그렇지 않다'가 포함되면 '그렇지 않다'도 매칭될 수 있음. Strict check needed?
            // Actually: "해당 문항에서 '전혀 그렇지 않다'에 응답해야 한다." contains "그렇지 않다".
            // ORDER MATTERS: Check "전혀 그렇지 않다" first.

            if (q.content.includes("'전혀 그렇지 않다'에 응답")) target = 1;
            else if (q.content.includes("'그렇지 않다'에 응답")) target = 2; // Check this AFTER "전혀"
